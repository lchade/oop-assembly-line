...
namespace sict {
    ...
    bool CustomerOrder::isFilled() const {
        bool result = true;
        for (size_t i = 0; i < m_noItems && result; i++) {
            if (!m_itemInfo[i].mm_filled) result = false;
        }
        return result;
    }
    ...
    //
    bool CustomerOrder::isItemFilled(const string& item) const {
        bool result = true;
        for (size_t i = 0; i < m_noItems && result; i++) {
    ...
    string CustomerOrder::getNameProduct() const {
        return string(m_custName) + "[" + string(m_prodName) + "]";
    }
